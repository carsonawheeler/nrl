...
export async function getLeagueData() {
  ...
  // ---- S1PICKS / S2PICKS: [pick_no, slug, name, owner] ----
  // Only S1/S2 have a draft-board view in the design; S3+ picks are stored in the
  // DB but not surfaced here, so they don't leak into the S2 view.
  const S1PICKS = [];
  const S2PICKS = [];
  ...
  return { TEAMS, PLAYERS, COACHES, MOVES, S1PICKS, S2PICKS, CHAMPS, RINGS, RESULTS, STATS };
}
