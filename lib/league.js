...
export async function getLeagueData() {
  const [teamRows, standings, roster, pstats, coachRows, moveRows, board, games, seasonRows, faRows, notifRows, gpstats, coachStatRows, coachFaRows] =
    await Promise.all([
      ...
      sql`SELECT c.name, c.awards AS coach_awards, se.number AS season, t.slug AS team, cs.role, cs.is_playoff,
  ...
  const coachPersonalAwards = {}; // name -> ['S3 Coach of the Year', ...]
  for (const r of coachStatRows) {
    ...
    if (r.coach_awards) coachPersonalAwards[r.name] = r.coach_awards;
  }
  ...
  for (const [name, seasonsObj] of Object.entries(coachSeasons)) {
    ...
    COACHSTATS[name] = {
      ...
      rows: rows.reverse(),
      career,
      personalAwards: coachPersonalAwards[name] || [],
      playerAwards,
    };
  }
  ...
  // "Current" is the latest season that actually has coach data, not the latest
  // season overall — an upcoming season (e.g. S4 added before it kicks off) has
  // no coach_season_stats yet, so keying off max(season.number) would treat
  // every coach as inactive and mislabel them all as free agents.
  const curSeasonNum = Math.max(0, ...Object.values(COACHSTATS).flatMap((cs) => cs.rows.map((r) => r.s)));
  ...
  // Official award winners for the current season, if recorded. Derived from the
  // same players.awards source that feeds the player pages and coach accolades,
  // so the home-page winners card can never disagree with them. When present,
  // `final` is true and the home page shows the winners in place of the live
  // race (which stays computed above so it reappears automatically next season).
  const userByName = {};
  for (const p of PLAYERS) userByName[p.n] = p.u;
  const winnerByType = {};
  for (const h of awardHolders) {
    if (!h.aw) continue;
    for (const award of h.aw) {
      const m = /^S(\d+)\s+(.+)$/.exec(award);
      if (m && Number(m[1]) === currentSeason) winnerByType[m[2].trim()] = h.nm;
    }
  }
  const winners = [['mvp', 'MVP'], ['dpoy', 'DPOY'], ['tmoy', 'TMOY'], ['soy', 'SOY'], ['roy', 'ROY']].map(
    ([code, label]) => {
      const name = winnerByType[label] || null;
      return name
        ? { code, label, n: name, t: (STATTEAMS[name] || {})[currentSeason] || null, u: userByName[name] || null }
        : { code, label, n: null };
    }
  );
  const AWARDS = {
